...
try {
  if (process.env.SUPABASE_URL && process.env.SUPABASE_SERVICE_ROLE_KEY) {
    ...
    // Use the same Supabase database as subscription management
    ...
    console.log("✅ Supabase connection initialized with unified database credentials");
  }
} catch (error) {
...
app.get('/api/database/tables', async (req, res) => {
  try {
    ...
    const pgClient = new Client({
      host: process.env.DB_HOST || 'db.ioqjonxjptvshdwhbuzv.supabase.co',
      port: parseInt(process.env.DB_PORT || '5432'),
      database: process.env.DB_NAME || 'postgres',
      user: process.env.DB_USERNAME || 'postgres',
      password: process.env.DB_PASSWORD || 'Leadsmarket#$@',
      ssl: {
...
app.post("/api/create-portal", async (req, res) => {
  try {
    ...
    const generated_webhook_url = `${process.env.BASE_URL || 'http://localhost:3000'}/api/webhooks/${portalCode}`;
...
app.post("/api/portals", async (req, res) => {
  try {
    ...
    const generated_webhook_url = `${process.env.BASE_URL || 'http://localhost:3000'}/api/webhooks/${portal_code}`;
